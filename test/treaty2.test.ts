...
describe('Treaty2 - Using endpoint URL', () => {
	...
	it('return $path', async () => {
		expect(treatyApp.nested.data['~path']).toEqual('/nested/data')
		expect(treatyApp.id({ id: '1' })['~path']).toEqual('/id/1')
	})
	...
	it('stream non-sse response', async () => {
		const responses = <any[]>[]

		const app = new Elysia().get('/generator', async function* () {
			yield 'a'
			yield { hello: 'world' }
			yield 1
			yield true
		...
		for await (const chunk of response.data!) responses.push(chunk)

		expect(responses).toEqual(['a', { hello: 'world' }, 1, true])
	})
})
